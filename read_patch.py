...
def getPatch(arg):
    ...
    img = cv2.imread(arg)
    bimg = cv2.medianBlur(img, 3)
    cimg = cv2.cvtColor(bimg, cv2.COLOR_BGR2GRAY)
    # find circles, pots
    circles = cv2.HoughCircles(cimg,cv2.HOUGH_GRADIENT,1,20,param1=40,param2=20,minRadius=23,maxRadius=30)
    ...
    if (template_points.shape[0] < circle_points.shape[0]):
        circle_points = circle_points[:template_points.shape[0]]
    else:
        template_points = template_points[:circle_points.shape[0]]
